...
Todos os subprojetos vivem dentro de `T1ESII/` no mesmo repositório:

```
T1-ES2/
└── T1ESII/
    ├── chave-infra-main/     ← docker-compose + Makefile + Terraform
    ├── chave-ms-auth-main/   ← API REST (Node.js / Express / JWT)
    ├── chave-mfe-auth-main/  ← Microfrontend (React / TS / MUI)
    ├── chave-shell-main/     ← Shell host (React Router + Module Federation)
    └── docs/                 ← ADRs, UI Manual, AI Usage
```

> O `docker-compose.yml` usa caminhos relativos (`../chave-ms-auth-main`, etc.),
> então os subprojetos **devem estar nessa estrutura** para o build funcionar.
...
O `make setup` faz: pull das imagens base → build dos containers → sobe PostgreSQL, MS Auth, MFE Auth e Shell.
...
# Retorna 200 com lista de usuários

curl -s http://localhost:3001/auth/admin/users \
  -H "Authorization: Bearer TOKEN_USER"
# Retorna 403 { "error": "Acesso negado: permissão insuficiente" }
```
...
### MS Auth — Jest + supertest (mock do banco)
...
O `.env` em `chave-infra-main/` controla todos os serviços. Copie o exemplo:

```bash
cp chave-infra-main/.env.example chave-infra-main/.env
```
...
|---|---|---|
| `JWT_SECRET` | `change-me-in-production` | Assina access tokens (troque em produção) |
| `JWT_REFRESH_SECRET` | *(não definido — usa fallback)* | Assina refresh tokens; defina explicitamente |
...
> Nunca use `change-me-in-production` em ambiente real.
...
### `remoteEntry.js` não encontrado (tela em branco no Shell)
...
- Aguarde o health check do Postgres (pode demorar ~20s na primeira vez)
...
### Reconstruir um serviço específico após mudança de código
